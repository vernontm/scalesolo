...
// Curated brand → local logo file map. The map is GENERATED by
// scripts/sync-brand-logos.mjs — re-run `npm run sync-logos` to refresh
// SVGs from Simple Icons and regenerate the alias table. Files live in
// /public/brand-logos/ and ship with the deploy.
//
// When a brand IS in this map, the renderer drops its hand-picked logo
// in at full card area (object-fit: cover) so it reads cleanly on
// video. When a brand is NOT in this map, we render the single-letter
// glyph fallback instead of hitting any remote favicon service.
import { LOCAL_BRAND_LOGOS } from './brand-logos-map.js'

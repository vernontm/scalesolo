...
// Per-template composition guidance. Variable slot names differ between
// templates (Sleek: title_chrome/title_accent; Atlas: title_pre/title_highlight),
// so Claude needs template-specific instructions. We pick the block by
// template_id with Sleek as fallback.
function buildCompositionGuidance(tmpl) {
  if (tmpl.id === 'atlas') {
...
- The items array passed to atlas-scene-list-v1 MUST be the literal
  ...
  // Sleek (default) — original guidance, unchanged.

              ...
              items: {
                ...
                properties: {
                  ...
                  zone: {
                    type: 'string',
                    enum: [...ALL_ZONES],
                    description: 'Where on the frame to place it. Side slots: left_overlay, right_overlay (one large card per side, vertically centered). lower-third for captions. top-strip is vertical-only.',
                  },
                  content: {
                    type: 'object',
...
- A stat the avatar speaks aloud ("we grew 10x", "47 thousand subscribers") → stat-callout-v1 in right_overlay with the actual number. Always include the unit suffix if it's natural ("10" + unit "x", "47K", "$2M").
- A specific company or tool the avatar names ("HeyGen", "Claude", "Notion", "Shopify", "OpenAI", any product brand) → tool-logo-v1 with logo glyph = the company's first letter (capital). Place in right_overlay so it sits next to the speaker. Name field is the full company name.
  ...
  try {
    ...
    try {
      ...
      if (!mapInput || !Array.isArray(mapInput.segments) || !mapInput.segments.length) {
        // Surface what Claude DID return so the failure is debuggable
        // instead of a black box. Most common causes: stop_reason=
        // "max_tokens" (system prompt + map too large), refusal/policy,
        // or schema mismatch where Claude returned text instead of a
        // tool_use block.
        const stopReason = claudeResp?.stop_reason || 'unknown'
        const textBlocks = (claudeResp?.content || [])
          .filter((b) => b.type === 'text')
          .map((b) => b.text)
          .join(' ')
          .slice(0, 400)
        const blockTypes = (claudeResp?.content || []).map((b) => b.type).join(',') || 'none'
        const segCount = Array.isArray(mapInput?.segments) ? mapInput.segments.length : 'n/a'
        throw new Error(
          `Claude did not return a usable video map ` +
          `(stop_reason=${stopReason}, blocks=${blockTypes}, segments=${segCount}). ` +
          `Sample text: ${textBlocks || '(none)'}`,
        )
      }

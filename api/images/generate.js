  ...
  try {
    const out = await anthropicMessage({
      ...
      system: [
        ...
        '',
        'CROSS-REFERENCE NEGATIVE BINDINGS (CRITICAL for multi-ref prompts):',
        '• If a PERSON ref and an OUTFIT ref both exist, append after the role expansions: "IDENTITY ISOLATION: The face, freckles, makeup, eyes, lips, jawline, and ALL facial features must come ONLY from reference \\"<PERSON-LABEL>\\". Do NOT pull any facial features, skin texture, or makeup style from reference \\"<OUTFIT-LABEL>\\". Treat reference \\"<OUTFIT-LABEL>\\" as headless clothing only — its face is irrelevant and must not influence the output face in any way."',
        '• If a PERSON ref and a BACKGROUND ref both exist, append: "Identity comes ONLY from reference \\"<PERSON-LABEL>\\". The person who appears in reference \\"<BACKGROUND-LABEL>\\", if any, is irrelevant — do NOT use their face or body. Only the environment from reference \\"<BACKGROUND-LABEL>\\"."',
        '• If multiple refs are PERSON role, the FIRST one in prompt order is the primary identity. Append: "Primary identity: reference \\"<FIRST-PERSON-LABEL>\\". Do not blend in features from any other PERSON reference unless the prompt explicitly asks."',
        '• Always inject the IDENTITY ISOLATION bindings near the top of the rewritten body, immediately after the per-ref role expansions. Place them BEFORE any descriptive scene details so they read as overriding rules.',
        '',
        'Disambiguation:',

// POST /api/carousels/ideas  { profile_id }
// Returns: { ideas: [string, string, string] }
//
// Three short carousel topic ideas in the brand's voice, to autofill the
// builder. Cheap text-only call; not credit-gated.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const out = await anthropicMessage({
      ...
      max_tokens: 500,
      system: [
        'Suggest 3 short, specific CAROUSEL topic ideas for this brand — the kind that make a strong save-worthy multi-slide post.',
        'Each idea is one line, <= 12 words, concrete (numbers/specifics welcome), in the brand voice.',
        'Return ONLY a JSON array of 3 strings. No preamble. NO em dashes.',
        brandMd ? `\nBrand context:\n${brandMd}` : '',
      ].filter(Boolean).join('\n'),
      messages: [{ role: 'user', content: 'Give me 3 carousel ideas.' }],
    })

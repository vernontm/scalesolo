...
Each email below has two sections:

1. **Body** — the exact text to paste into the plain-text builder
2. **Links to apply** — which words inside the body to hyperlink, and to what URL

In the MailerLite builder, after pasting the body: select each phrase
shown in the "Links to apply" table, click the link icon, paste the URL.
The `>>` and `<<` characters stay as plain text — do not include them
in your hyperlink selection.
...
**Tracked download URLs** — every download link routes through
`/api/r/<asset>` so we log every click in `contact_activity`. Each
email has its own `src=<email-id>` so we can attribute clicks back to
the specific email in the activity log.
...
| Select this text | Link to |
|------------------|---------|
| `Open the Blueprint` | `https://www.scalesolo.ai/api/r/blueprint?src=lead-day1&e={$email}` |
| `right here` (in "pop the Blueprint back open right here") | same URL as above |
...
| Select this text | Link to |
|------------------|---------|
| `right here` (in "open it again right here") | `https://www.scalesolo.ai/api/r/blueprint?src=lead-day2&e={$email}` |
| `See the playbook (Build Your AI Empire, $17)` | `https://www.scalesolo.ai/build-your-ai-empire` |
| `Grab it` (in the final "Grab it before it gets buried" sentence) | `https://www.scalesolo.ai/build-your-ai-empire` |
...
| Select this text | Link to |
|------------------|---------|
| `Get the playbook for $17` | `https://www.scalesolo.ai/build-your-ai-empire` |
| `here` (in "you can grab it here") | `https://www.scalesolo.ai/build-your-ai-empire` |
...
is your sign. You can grab the playbook right here or lock in
Founding over here.
```
...
| Select this text | Link to |
|------------------|---------|
| `Get the playbook ($17)` | `https://www.scalesolo.ai/build-your-ai-empire` |
| `Lock in Founding with SCALE` | `https://www.scalesolo.ai/welcome?product=tripwire` |
| `right here` (in "grab the playbook right here") | `https://www.scalesolo.ai/build-your-ai-empire` |
| `over here` (in "lock in Founding over here") | `https://www.scalesolo.ai/welcome?product=tripwire` |
...
the MailerLite Day 0 email. Leave both as-is for now. If you want to
dedupe, see principle 19 in the skill — the cleanest move is to delete
the MailerLite Day 0 and let the Resend Stripe receipt be the sole
confirmation.
...
| Select this text | Link to |
|------------------|---------|
| `Open the playbook` | `https://www.scalesolo.ai/api/r/playbook?src=tripwire-buyer-day1&e={$email}` |
| `right here` (in P.S., "revisit it any time right here") | same URL as above |
...
| Select this text | Link to |
|------------------|---------|
| `See the Done-For-You launch` | `https://www.scalesolo.ai/done-for-you` |
| `right here` (in "you can check it out right here") | `https://www.scalesolo.ai/done-for-you` |
...
|------------------|---------|
| `Lock in Founding with SCALE` | `https://www.scalesolo.ai/welcome?product=tripwire` |
| `right here` (in "lock it in right here") | `https://www.scalesolo.ai/welcome?product=tripwire` |
...
| Select this text | Link to |
|------------------|---------|
| `Fill out the 8-minute intake` | `https://vernontm.com/dfy-intake` |
| `right here` (in "you can do that right here") | `https://vernontm.com/book-call` |
...
If you want a walkthrough of how to use the auto-posting workflow
once it's live, here you go:
...
| Select this text | Link to |
|------------------|---------|
| `Watch the workflow walkthrough` | `https://vernontm.com/dfy-walkthrough` |
...
**Links to apply:** None. This email is a pure reply prompt — leave it text-only with no hyperlinks.
...
| Select this text | Link to |
|------------------|---------|
| `Take another look at the playbook` | `https://www.scalesolo.ai/build-your-ai-empire` |
...
| Select this text | Link to |
|------------------|---------|
| `The playbook lives here` | `https://www.scalesolo.ai/build-your-ai-empire` |
...
| Select this text | Link to |
|------------------|---------|
| `See the playbook ($17)` | `https://www.scalesolo.ai/build-your-ai-empire` |
| `right here` (in "you can grab it right here") | `https://www.scalesolo.ai/build-your-ai-empire` |
...
- [ ] For each row in the "Links to apply" table: select the exact text
      in the body, click the link icon, paste the URL
...
characters (no underline, no link color). The text between them
should appear underlined or colored as a link. If the whole
`>> Anchor <<` string is underlined, you accidentally included the
chevrons in the hyperlink selection — undo and reselect just the
inner text.
